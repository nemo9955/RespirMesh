    ...
    void recvParentObj()
    {
        ...
        while (tcpParent.receive())
        {
            ...
            if (tcpParent.msgLen > 0)
            {
                ...
                this->received((uint8_t *)tcpParent.msg, tcpParent.msgLen);
                tcpParent.clean();
            }
            // logf(".");
        }
  ...
  public:
    ...
    void recvParent()
    {
        while (1)
        {
            ...
            client_->set_receiver(this->on_recv_cb, argv_recv.argv);
            channels.push_back((RemChannel *)client_);
...
RemRouter mesh(&hardware);

int main(int argc, char *argv[])
{
    const char *IP = "127.0.0.1";
    signal(SIGINT, sig_exit);
    debugf("STARTING CLIENT \n");

    if (argc < 3)
    {
        debugf("First argument specify the port to connect to \n");
        debugf("Second argument specify a port to open the server to\n");
        exit(1);
    }
...
void *loop(void *m)
{
    ...
    while (1)
    {
        ...
        if (len > 0)
        {
            ...
            debugf("  ForwardingType %d \n", header->ForwardingType);
            debugf("      HeaderType %d \n", header->HeaderType);
            debugf("    ProtobufType %d \n", header->ProtobufType);
            ...
            else if (header->ForwardingType == ForwardingType_TO_PARENT_TO_ROOT)
            {
                ...
                case ProtobufType_MESH_TOPOLOGY:
                    debugf("putting data in  TOPO to root : \n");
            ...
            for (uint8_t i = 0; i < len; i++)
                debugf(" %d", tcpServer.msg[i]);
            // debugf("%d ", str.c_str()[i]);
            debugf("\n");
